...
>
  ...
  <!-- Expand arrow for cards with subs -->
  {#if card.subs?.length}
...
.game-icon :global(text) { transition: inherit; }

.game-card:hover .game-icon {
  color: var(--game-color);
  filter: drop-shadow(0 0 6px var(--game-glow));
}
...
.game-card.selected {
  border-color: var(--game-color);
  background: color-mix(in srgb, var(--game-color) 8%, #1a1a1a);
}
.game-card.selected .card-name {
  color: var(--game-color);
}
</style>
